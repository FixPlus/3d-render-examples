...
namespace APITest{
    ...
    public:
        ...
        renderInterface_(renderInterface), windowInterface_(dynamic_cast<WindowInterface*>(renderInterface->getWindow().get())){
            if(!windowInterface_)
                throw std::runtime_error("[GUI][ERROR]: could not create GUI if no window attached to renderer.");
        }

        /** Updates the state of UI: reads specific window data, collecting setting user input information, etc.*/
        ...
        /** this callbacks are called from RenderInterface callback processing functions. */
        /** Full hierarchy: WindowCallback -> RenderCallback -> GUICallback -> User Defined callback. */
