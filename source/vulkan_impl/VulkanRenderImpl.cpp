...
APITest::VulkanRenderImpl::VulkanRenderImpl(APITest::WindowRef const& window, bool enableValidation):
    window_(window), validationLayersEnable(enableValidation), windowInterface(dynamic_cast<WindowInterface*>(window.get())) {
    ...
    if(window_) {
        swapChain = new VulkanSwapChain(this, windowInterface->getSurface(instance));
        swapChain->recreate(&windowInterface->width_, &windowInterface->height_, false);
...
bool APITest::VulkanRenderImpl::render() {
    ...
    if(swapChain && renderJob){
        ...
        if(swapChain->submitAndPresent(commandBuffers.data() + imageIndex) != VK_SUCCESS){
            windowResized();
        }
    }
...
void APITest::VulkanRenderImpl::createCommandBuffers() {
    ...
    queueManager->createPrimaryCommandBuffers(commandBuffers.data(), commandBuffers.size());
...
static VkShaderStageFlagBits convertVkStage(APITest::ShaderStage stage){
    switch(stage){
        case APITest::ShaderStage::VERTEX: return VK_SHADER_STAGE_VERTEX_BIT;
        case APITest::ShaderStage::FRAGMENT: return VK_SHADER_STAGE_FRAGMENT_BIT;
        default:
...
APITest::ShaderRef APITest::VulkanRenderImpl::createShaderProgram(const APITest::ShaderDesc &shaderDesc) {
    return shaderManager->loadShader(shaderDesc.filename, convertVkStage(shaderDesc.stage), shaderDesc.filename);
}
...
APITest::GraphicsPipelineRef APITest::VulkanRenderImpl::createGraphicsPipeline(APITest::GraphicsPipelineLayout layout) {
    return pipelineManager->get(layout);
}

void APITest::VulkanRenderImpl::waitIdle() {
    device->waitIdle();
}

APITest::VertexBufferRef APITest::VulkanRenderImpl::createVertexBuffer(void* initialData, size_t initialSize,
                                   APITest::MemoryType memType) {
    auto ret = std::unique_ptr<VertexBuffer>(new VulkanVertexBuffer(memoryManager, memType, initialSize));
...
APITest::IndexBufferRef APITest::VulkanRenderImpl::createIndexBuffer(void* initialData, size_t initialSize, APITest::IndexBuffer::Type indexType,
                                                                       APITest::MemoryType memType) {
    auto ret = std::unique_ptr<IndexBuffer>(new VulkanIndexBuffer(memoryManager, memType, initialSize, indexType));
...
void APITest::VulkanRenderImpl::pushNewRenderPassGraph(APITest::RenderPassRef node) {
    delete renderJob;
...
APITest::OnscreenRenderPassRef APITest::VulkanRenderImpl::createOnscreenColorPass() {
...
APITest::VulkanRenderImpl::createDescriptorLayout(const std::vector<DescriptorLayout> &desc) {
    return descriptorManager->createNewDescriptorSetLayout(desc);
}

APITest::SamplerRef APITest::VulkanRenderImpl::createSampler(const APITest::SamplerDesc &desc) {
    return SamplerRef (new VulkanSampler(this, desc));
}

APITest::UniformBufferRef
APITest::VulkanRenderImpl::createUniformBuffer(void *initialData, size_t initialSize, APITest::MemoryType memType) {
    auto buffer = APITest::UniformBufferRef(new VulkanUniformBuffer(memoryManager, memType, initialSize));
    if(initialData)
