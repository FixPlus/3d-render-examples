...
APITest::VulkanPipelineManager::VulkanPipelineManager(APITest::VulkanRenderImpl *parent): parent_(parent) {
    ...
    VK_CHECK_RESULT(vkCreatePipelineCache(parent->getVulkanDevice()->get(), &pipelineCacheCreateInfo, nullptr, &pipelineCache));
}
...
static std::pair<uint32_t, std::vector<VkVertexInputAttributeDescription>> vertexStateCI(APITest::GraphicsPipelineLayout const& desc){
...
APITest::GraphicsPipelineRef APITest::VulkanPipelineManager::get(APITest::GraphicsPipelineLayout const& desc){
    return std::unique_ptr<GraphicsPipeline>(new VulkanGraphicsPipeline(this, desc));
}


APITest::VulkanGraphicsPipeline::VulkanGraphicsPipeline(APITest::VulkanPipelineManager* parent, APITest::GraphicsPipelineLayout layout):
VulkanPipeline(parent), layout_(std::move(layout)){

    VkDescriptorSetLayout setLayout = VK_NULL_HANDLE;
    if(auto* vkSetLayout = dynamic_cast<VulkanDescriptorSetLayout*>(layout_.descriptorsLayout.get())){
        setLayout = vkSetLayout->layout();
    }
    VkPipelineLayoutCreateInfo pPipelineLayoutCreateInfo =
            initializers::pipelineLayoutCreateInfo(
                    &setLayout,
                    setLayout != VK_NULL_HANDLE ? 1 : 0);

    VK_CHECK_RESULT(vkCreatePipelineLayout(parent_->parent_->getVulkanDevice()->get(),
                                           &pPipelineLayoutCreateInfo, nullptr, &layoutHandle));

}

static VkCompareOp convertToVkCompareOp(APITest::RasterizerLayout::DepthTest::CompOp compOp){
    switch (compOp) {
        case APITest::RasterizerLayout::DepthTest::CompOp::LESS_OR_EQUAL: return VK_COMPARE_OP_LESS_OR_EQUAL;
        case APITest::RasterizerLayout::DepthTest::CompOp::LESS: return VK_COMPARE_OP_LESS;
        case APITest::RasterizerLayout::DepthTest::CompOp::GREATER: return VK_COMPARE_OP_GREATER;
        case APITest::RasterizerLayout::DepthTest::CompOp::GREATER_OR_EQUAL: return VK_COMPARE_OP_GREATER_OR_EQUAL;
        default: return VK_COMPARE_OP_MAX_ENUM;
    }
}

static VkCullModeFlags convertCullMode(APITest::RasterizerLayout::CullingState::CullMode mode){
    switch (mode) {
        case APITest::RasterizerLayout::CullingState::CULL_NONE : return VK_CULL_MODE_NONE;
        case APITest::RasterizerLayout::CullingState::CULL_FRONT_FACE : return VK_CULL_MODE_FRONT_BIT;
        case APITest::RasterizerLayout::CullingState::CULL_BACK_FACE : return VK_CULL_MODE_BACK_BIT;
    }
    return VK_CULL_MODE_FRONT_AND_BACK;
}

static VkFrontFace convertFrontFace(APITest::RasterizerLayout::CullingState::FrontFace face){
    if(face == APITest::RasterizerLayout::CullingState::FRONT_FACE_CCW)
        return VK_FRONT_FACE_COUNTER_CLOCKWISE;
    else
        return VK_FRONT_FACE_CLOCKWISE;
}
VkPipeline APITest::VulkanGraphicsPipeline::get(APITest::VulkanRenderPass* pass) {

    auto* colorPass = dynamic_cast<VulkanColorPass*>(pass);

    if(!colorPass)
        throw std::runtime_error("[VULKAN][ERROR]: trying to bind graphics pipeline to non-color render pass.");
    ...
    VkPipelineRasterizationStateCreateInfo rasterizationState =
            initializers::pipelineRasterizationStateCreateInfo(VK_POLYGON_MODE_FILL,
                                                               ...
                                                               convertFrontFace(layout_.rasterizerLayout.cullingState.face),
                                                               0);
    ...
    if(layout().rasterizerLayout.depthTest.enable && !colorPass->hasDepthBuffer())
        throw std::runtime_error("[VULKAN][ERROR]: binding graphics pipeline with depth test enabled to color pass without depth buffer allocated.");

    VkPipelineDepthStencilStateCreateInfo depthStencilState =
            initializers::pipelineDepthStencilStateCreateInfo(layout().rasterizerLayout.depthTest.enable ? VK_TRUE : VK_FALSE,
    ...
    VkGraphicsPipelineCreateInfo pipelineCI = initializers::pipelineCreateInfo(layoutHandle, vkPass, 0);
    ...
    VkVertexInputBindingDescription vertexAttrBindingInfo = VkVertexInputBindingDescription({ 0, stride, VK_VERTEX_INPUT_RATE_VERTEX });
    ...
    shaderStages[0].module = instance.getShaderManager()->getModule(layout_.vertexLayout.vertexShader);
    ...
    shaderStages[1].module = instance.getShaderManager()->getModule(layout_.fragmentLayout.fragmentShader);
    ...
    VK_CHECK_RESULT(
            vkCreateGraphicsPipelines(instance.getVulkanDevice()->get(), parent_->pipelineCache, 1, &pipelineCI, nullptr, &newPipeline));
