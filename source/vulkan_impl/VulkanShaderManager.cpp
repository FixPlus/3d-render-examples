...
uint32_t
APITest::VulkanShaderManager::loadShader(const std::string &filename,
    ...
    if (is.is_open())
    {
        ...
        if(size == 0)
            throw std::runtime_error("[VK][ERROR]: shader module load failed: file is empty. Filename: " +
                                            filename);
        ...
        VK_CHECK_RESULT(vkCreateShaderModule(parent_->getVulkanDevice()->get(), &moduleCreateInfo,
                                             NULL, &shaderModule))
    ...
    {
        throw std::runtime_error("[VK][ERROR]: shader module load failed: could not open file. Filename: " + filename);
    }
...
APITest::VulkanShaderManager::~VulkanShaderManager() {
    clear();
}
