...
APITest::RenderJob::RenderJob(APITest::VulkanRenderImpl *parent, RenderPassRef desc): parent_(parent) {
...
void APITest::RenderJob::compileRenderPass(RenderPassRef desc) {
    ...
    if(auto* onScreenPass = dynamic_cast<VulkanOnscreenRenderPass*>(vkPass)){
        ...
        for(int i = 0; i < imageCount; i++){
            unit.frameBuffers.push_back(onScreenPass->allocateFrameBuffer( swapChain->buffers.at(i).view));
        }
    } else {
        throw std::runtime_error("[Vulkan][ERROR] Render Pass graph compilation failed: offscreen passes aren't supported yet.");
    }
...
void APITest::RenderJob::resetFrameBuffers() {
    for(auto& unit: renderSequence){
        ...
        if(auto* onScreenPass = dynamic_cast<VulkanOnscreenRenderPass*>(pass.get())){
            for(auto buffer: unit.frameBuffers)
                vkDestroyFramebuffer(parent_->getVulkanDevice()->get(), buffer, nullptr);
            unit.frameBuffers.clear();
            auto* swapChain = parent_->getSwapChain();
            int imageCount = swapChain->buffers.size();
            onScreenPass->changeExtents(swapChain->surfaceWidth, swapChain->surfaceHeight);
            for(int i = 0; i < imageCount; i++){
                unit.frameBuffers.push_back(onScreenPass->allocateFrameBuffer(swapChain->buffers.at(i).view));
            }
        }
...
void APITest::RenderJob::compile(int swapChainImage, VkCommandBuffer commandBuffer) {
    ...
    for(auto& unit: renderSequence){
        ...
        auto* vkPass = dynamic_cast<VulkanRenderPass*>(pass.get());
        auto currentBuffer = unit.frameBuffers.size() == 1 ? unit.frameBuffers.front() : unit.frameBuffers.at(swapChainImage);
