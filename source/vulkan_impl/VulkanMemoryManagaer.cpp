...
namespace APITest {
    ...
    VkImageCreateInfo compileDepthStencilImageCreateInfo(VulkanDepthBufferCI createInfo){
        VkImageCreateInfo ret = initializers::imageCreateInfo();
        ret.format = createInfo.depthFormat;
        ret.imageType = VK_IMAGE_TYPE_2D;
        ret.usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | createInfo.additionalUsage;
        ret.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        ret.samples = VK_SAMPLE_COUNT_1_BIT;
        ret.mipLevels = 1;
        ret.arrayLayers = 1;
        ret.tiling =  createInfo.additionalUsage == 0 ? VK_IMAGE_TILING_OPTIMAL : VK_IMAGE_TILING_LINEAR;
    ...
    VulkanDepthBuffer::VulkanDepthBuffer(const VulkanMemoryManager *alloc, VulkanDepthBufferCI createInfo):
                            VulkanImage(alloc, MemoryType::GPU_PRIVATE,
                                        static_cast<VkBufferUsageFlagBits>(VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
        ...
        VK_CHECK_RESULT(vkCreateImageView(alloc->device(), &viewInfo, nullptr, &depthView_))
    }
    ...
    VkImageView VulkanDepthBuffer::getDepthView() const {
        return depthView_;
    }

    Image::Type VulkanImageInterface::getType() const {
        switch (imageInfo_.imageType) {
            case VK_IMAGE_TYPE_1D: return Image::Type::TEXTURE_1D;
            case VK_IMAGE_TYPE_2D: return Image::Type::TEXTURE_2D;
            case VK_IMAGE_TYPE_3D: return Image::Type::TEXTURE_3D;
            default: return Image::Type::INVALID;
        }
    }

    Image::Format VulkanImageInterface::getFormat() const {
        switch(imageInfo_.format){
            case VK_FORMAT_R8G8B8A8_SRGB: return Image::Format::RGBA8;
            case VK_FORMAT_D24_UNORM_S8_UINT: return Image::Format::D24S8;
            case VK_FORMAT_B10G11R11_UFLOAT_PACK32: return Image::Format::R11G11B10;
            default: return Image::Format::INVALID;
        }
    }

    Image::Extents VulkanImageInterface::getImageExtents() const {
        return {imageInfo_.extent.width, imageInfo_.extent.height, imageInfo_.extent.depth};
    }
    ...
    void VulkanImageInterface::load(const void *data) {
        ...
        if(data == nullptr)
            throw std::runtime_error("[VULKAN][ERROR]: passed nullptr to image->load().");
    ...
    void VulkanImageInterface::createDefaultImageView() {
        ...
        switch(imageInfo_.imageType){
            case VK_IMAGE_TYPE_1D:
                createInfo.viewType = imageInfo_.arrayLayers > 1 ? VK_IMAGE_VIEW_TYPE_1D_ARRAY: VK_IMAGE_VIEW_TYPE_1D;
                break;
            case VK_IMAGE_TYPE_2D:
                createInfo.viewType = imageInfo_.arrayLayers > 1 ? VK_IMAGE_VIEW_TYPE_2D_ARRAY: VK_IMAGE_VIEW_TYPE_2D;
                break;
    ...
    VkFormat convertInterfaceImageFormat(Image::Format format){
        switch (format) {
            ...
            case Image::Format::R11G11B10: return VK_FORMAT_B10G11R11_UFLOAT_PACK32;
            case Image::Format::D24S8: return VK_FORMAT_D24_UNORM_S8_UINT;
            default: return VK_FORMAT_UNDEFINED;
        }
    }
    VkImageType convertInterfaceImageType(Image::Type type){
        switch (type) {
            case Image::Type::TEXTURE_1D: return VK_IMAGE_TYPE_1D;
            case Image::Type::TEXTURE_2D: return VK_IMAGE_TYPE_2D;
            case Image::Type::TEXTURE_3D: return VK_IMAGE_TYPE_3D;
    ...
    VkImageCreateInfo compileGeneralVulkanImageCI(ImageDesc desc){
        ...
        if(desc.type != Image::Type::TEXTURE_3D && desc.extents.depth != 1)
            throw std::runtime_error("[VULKAN][ERROR] if Image::Type != TEXTURE_3D depth must be equal 1");
        if(desc.type == Image::Type::TEXTURE_1D && desc.extents.height != 1)
            throw std::runtime_error("[VULKAN][ERROR] if Image::Type == TEXTURE_1D height must be equal 1");
        ...
        ret.tiling = desc.memType == MemoryType::GPU_PRIVATE ? VK_IMAGE_TILING_OPTIMAL : VK_IMAGE_TILING_LINEAR;
        ret.flags = 0;
        ret.mipLevels = 1;
        return ret;
    }

    VulkanGeneralImage::VulkanGeneralImage(VulkanMemoryManager const* alloc, ImageDesc desc):
                                    VulkanImage(alloc, desc.memType,
                                                static_cast<VkBufferUsageFlagBits>(convertInterfaceImageUsageFlags(
                                                        desc.usage)), compileGeneralVulkanImageCI(desc)) {

    }

    VulkanStagingBuffer::VulkanStagingBuffer(const VulkanMemoryManager *alloc, size_t size): VulkanBuffer(alloc, MemoryType::HOST_COHERENT, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,  size) {

    }
}

APITest::VulkanBuffer::VulkanBuffer(VulkanMemoryManager const* alloc, APITest::MemoryType type, VkBufferUsageFlagBits usage, size_t size): Allocatable(alloc), usage_(usage) {

    if(size == 0)
        throw std::runtime_error("[VULKAN][ERROR] trying to create buffer with size == 0 which is prohibited.");

    size_ = size;
    // If allocating in private memory, pushing data will involve explicit data transfer operation
    ...
    vmaCreateBuffer(memoryManager->allocator(), &bufferCI, &allocInfo, &buffer_, &allocation, &allocationInfo);
...
void APITest::VulkanBuffer::push(const void *data, size_t size, size_t offset) {
    ...
    if(size + offset > allocationInfo.size)
        throw std::runtime_error("[VULKAN][ERROR] pushing to out of bounds memory. " +
    ...
    if(allocationInfo.memoryType | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT){
        ...
        vmaCreateBuffer(memoryManager->allocator(), &bufferCI, &allocInfo, &stagingBuffer, &stagingAllocation, &stagingBufferInfo);
        ...
        vmaDestroyBuffer(memoryManager->allocator(), stagingBuffer, stagingAllocation);
    }
}
...
void APITest::VulkanMemoryManager::copy(VkBuffer src, VkBuffer dst, VkBufferCopy region) const{
...
void APITest::VulkanMemoryManager::transitLayout(APITest::VulkanImage *image, VkImageLayout newLayout) const {
    ...
    parent_->getQueueManager()->flush(transitCmd, parent_->getQueueManager()->getTransferQueue(), true);

}

void APITest::VulkanMemoryManager::copy(APITest::VulkanBuffer *buffer, APITest::VulkanImage *image) const {
    ...
    parent_->getQueueManager()->flush(copyCmd, parent_->getQueueManager()->getTransferQueue(), true);


}

APITest::VulkanImage::VulkanImage(const APITest::VulkanMemoryManager *alloc, APITest::MemoryType type,
                                  VkBufferUsageFlagBits usage, VkImageCreateInfo CI): Allocatable(alloc), usage_(usage), imageInfo_(CI), currentLayout_(CI.initialLayout) {

    VmaAllocationCreateInfo allocInfo = chooseMemoryOptions(type);

    VK_CHECK_RESULT(vmaCreateImage(memoryManager->allocator(), &CI, &allocInfo, &image_, &allocation, &allocationInfo))
