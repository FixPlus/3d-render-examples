...
APITest::VulkanDevice::VulkanDevice(const APITest::VulkanRenderImpl *parent, bool useSwapChain): parent_(parent) {
    ...
    vkEnumeratePhysicalDevices(parent_->getInstance(), &deviceCount, availableDevices.data());
...
void APITest::VulkanDevice::queryPhysicalDeviceInfo() {
    ...
    std::cout << "Vulkan picked following GPU for a test: " << properties.deviceName << std::endl;
...
uint32_t APITest::VulkanDevice::getQueueFamilyIndex(VkQueueFlagBits queueFlags) const {
...
VkFormat APITest::VulkanDevice::getSupportedDepthFormat(bool checkSamplingSupport) const {
