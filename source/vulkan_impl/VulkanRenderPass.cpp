...
APITest::VulkanOnscreenRenderPass::VulkanOnscreenRenderPass(VulkanRenderImpl* parent):
        VulkanRenderPass(parent) {
...
void APITest::VulkanOnscreenRenderPass::begin(VkCommandBuffer cmdBuffer, VkFramebuffer framebuffer) {
    ...
    VkViewport viewport = initializers::viewport((float)extents.width, (float)extents.height, 0.0f, 1.0f);
...
VkFramebuffer APITest::VulkanOnscreenRenderPass::allocateFrameBuffer(VkImageView swapChainImage) {
    ...
    VkFramebuffer ret;
    VK_CHECK_RESULT(vkCreateFramebuffer(parent_->getVulkanDevice()->get(), &frameBufferCreateInfo, nullptr,
                                        &ret))
    return ret;
}

void APITest::VulkanOnscreenRenderPass::changeExtents(int newWidth, int newHeight) {
    extents.width = newWidth;
    extents.height = newHeight;

    if(depthBuffer) {
        // depth buffer extents should match, so we need to reallocate image to trunc it to new size
        delete depthBuffer;
        depthBuffer = new VulkanDepthBuffer(parent_->getMemoryManager(),
                                            {parent_->getVulkanDevice()->getSupportedDepthFormat(
                                                    false), {extents.width, extents.height}, 0});
    }
}

void APITest::VulkanOnscreenRenderPass::enableDepthBuffer(uint32_t binding) {
    if(!depthBuffer){
        depthBuffer = new VulkanDepthBuffer(parent_->getMemoryManager(), {parent_->getVulkanDevice()->getSupportedDepthFormat(
                false), {extents.width, extents.height}, 0});
    }
...
static VkAttachmentLoadOp convertToVulkanLoadOp(APITest::OffscreenRenderPass::LoadOp loadop){
    switch (loadop) {
        case APITest::OffscreenRenderPass::LoadOp::LOAD: return VK_ATTACHMENT_LOAD_OP_LOAD;
        case APITest::OffscreenRenderPass::LoadOp::DONT_CARE: return VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        case APITest::OffscreenRenderPass::LoadOp::CLEAR: return VK_ATTACHMENT_LOAD_OP_CLEAR;
    }

    return VK_ATTACHMENT_LOAD_OP_DONT_CARE;
}

static VkAttachmentStoreOp convertToVulkanStoreOp(APITest::OffscreenRenderPass::StoreOp storeOp){
    switch (storeOp) {
        case APITest::OffscreenRenderPass::StoreOp::STORE: return VK_ATTACHMENT_STORE_OP_STORE;
        case APITest::OffscreenRenderPass::StoreOp::DONT_CARE : return VK_ATTACHMENT_STORE_OP_DONT_CARE;
    }
...
void APITest::VulkanOnscreenRenderPass::compile() {
    ...
    if(depthBuffer){
        ...
        attachmentsDesc[1].finalLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
