...
void APITest::StatisticsRecorder::endFrame(){
    ...
    double overallFrameTime = std::chrono::duration<double, std::milli>(frameBlock.childs.back()->stamp - frameBlock.childs.front()->stamp).count();
    ...
    std::stack<std::pair<std::pair<FrameBlock*, FrameBlock*>, int>> traversalStack;
    ...
    for(int i = 0; i < topLevelRecordsSize - 1; ++i){
        ...
        while (!traversalStack.empty()){
            ...
            if(currentChild < currentBlock->childs.size()){
                ...
                traversalStack.push({{currentBlock->childs.at(currentChild).get(),
                                      currentChild == currentBlock->childs.size() - 1 ? nextBlock : currentBlock->childs.at(currentChild + 1).get()}, 0});
                continue;
            }

            double blockTime = std::chrono::duration<double, std::milli>(nextBlock->stamp - currentBlock->stamp).count();
    ...
    if(totalFrames % 100 == 5) {
        std::stack<std::pair<std::pair<FrameBlock*, FrameStatistics::TimeBlock*>, int>> statStack;
        statistics.frameTimeBlock.duration.first = frameBlock.avgTime.record();
        statistics.frameTimeBlock.duration.second = 1000.0 / statistics.frameTimeBlock.duration.first;
        ...
        while(!statStack.empty()){
            ...
            if(currentFrameBlock->childs.size() != currentStatBlock->children.capacity())
            ...
            if(currentChild < currentFrameBlock->childs.size()){
                ...
                if(currentStatBlock->children.size() <= currentChild){
                    currentStatBlock->children.emplace_back(std::make_unique<FrameStatistics::TimeBlock>());
                }
                statStack.push({{currentFrameBlock->childs.at(currentChild).get(), currentStatBlock->children.at(currentChild).get()}, 0});
                continue;
            }

            currentStatBlock->name = currentFrameBlock->name;
            currentStatBlock->duration = {currentFrameBlock->avgTime.record(), currentFrameBlock->avgTime.record() / frameBlock.avgTime.record() * 100.0};
            statStack.pop();
        }
        statistics.frameTimeBlock.children.resize(statistics.frameTimeBlock.children.size() - 1);
