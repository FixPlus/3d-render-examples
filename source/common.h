...
namespace APITest{
    ...
    class StatisticsRecorder{
        ...
        template<typename T>
        static void addMetricInternal_(MetricsContainer<T>& container, std::string const& metricName, T metric){
        ...
        void addTimeStampInternal_(std::string const& blockName){
            ...
            if(currentFrame->childs.size() <= currentChild){
                ...
                if(savedInterval->name == blockName){
                    ...
                    currentFrame->childs.emplace(currentFrame->childs.begin() + currentChild, std::make_unique<FrameBlock>(blockName));
                }
            }
    ...
    public:
        ...
        void stamp(std::string const& blockName){
            addTimeStampInternal_(blockName);
        }
        ...
        template<typename T>
        std::enable_if_t<std::is_integral<T>::value, void> addMetric(std::string const& metricName, T metric){
            addMetricInternal_<T>(integerMetrics_, metricName, metric);
        }

        template<typename T>
        std::enable_if_t<std::is_floating_point<T>::value, void> addMetric(std::string const& metricName, T metric){
            addMetricInternal_<T>(floatMetrics_, metricName, metric);
        }

        FrameStatistics const& stat() const{
            return statistics;
        }
    };
}
