...
WindowInterface(desc.width, desc.height, desc.title) {
    ...
    if(glfwInit() != GLFW_TRUE)
        throw std::runtime_error("Failed to initialize GLFW. glfwInit() returned" + std::to_string(glfwGetError(nullptr)));

    // TODO: glfwSetErrorCallback(error_callback);

    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);

    instance_ = glfwCreateWindow(width_, height_,
                                 title_.c_str(), nullptr, nullptr);

    windowMap.emplace(instance_, this);


     // Framebuffer size can actually be different from the size in screen coordinates (from which window is created
     // from). Most time it's not the case but it will be fatal if mess up.
...
VkSurfaceKHR APITest::GLFWWindowImpl::getSurface(VkInstance vkInstance) {
    ...
    if(auto err = (glfwCreateWindowSurface(vkInstance, instance_, nullptr, &surface) - VK_SUCCESS))
        throw std::runtime_error("[GLFW ERROR] Could not create Vulkan surface. Error code: "
...
void APITest::GLFWWindowImpl::windowSizeChanged(GLFWwindow* window, int width, int height){
...
void APITest::GLFWWindowImpl::errorHandle(int error, const char* errorString){
    throw std::runtime_error("[GLFW][ERROR] GLFW library encountered following error: " +
    std::string(errorString) + "\nError code: " + std::to_string(error));
}
void APITest::GLFWWindowImpl::swapBuffers() {
...
bool APITest::GLFWWindowImpl::cursorEnabled() const {
...
void APITest::GLFWWindowImpl::keyInput(GLFWwindow *window, int key, int scancode, int action, int mods) {
...
void APITest::GLFWWindowImpl::cursorPosition(GLFWwindow *window, double xPos, double yPos) {
...
void APITest::GLFWWindowImpl::scrollInput(GLFWwindow *window, double xoffset, double yoffset) {
