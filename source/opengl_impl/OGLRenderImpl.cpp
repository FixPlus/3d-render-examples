...
APITest::OGLRenderImpl::OGLRenderImpl(APITest::WindowRef window): window_(std::move(window))
...
static GLenum convertStages(APITest::ShaderStage stage){
    switch(stage){
        case APITest::ShaderStage::VERTEX: return GL_VERTEX_SHADER;
        case APITest::ShaderStage::FRAGMENT: return GL_FRAGMENT_SHADER;
...
APITest::ShaderRef APITest::OGLRenderImpl::createShaderProgram(const APITest::ShaderDesc &shaderDesc) {
    return shaderManager->loadShader(shaderDesc.filename, convertStages(shaderDesc.stage));
}
...
APITest::GraphicsPipelineRef APITest::OGLRenderImpl::createGraphicsPipeline(APITest::GraphicsPipelineLayout layout) {
...
void APITest::OGLRenderImpl::pushNewRenderPassGraph(APITest::RenderPassRef node) {
...
APITest::OnscreenRenderPassRef APITest::OGLRenderImpl::createOnscreenColorPass() {
    return APITest::OnscreenRenderPassRef(new OGLOnscreenRenderPass());
}

APITest::WindowRef APITest::OGLRenderImpl::getWindow() const {
    return window_;
}
...
APITest::SamplerRef APITest::OGLRenderImpl::createSampler(const APITest::SamplerDesc &desc) {

    return APITest::SamplerRef(new OGLSampler(desc));
}

APITest::DescriptorSetLayoutRef
APITest::OGLRenderImpl::createDescriptorLayout(const std::vector<DescriptorLayout> &desc) {
    return DescriptorSetLayoutRef(new OGLDescriptorSetLayout(desc));
}

APITest::UniformBufferRef
APITest::OGLRenderImpl::createUniformBuffer(void *initialData, size_t initialSize, APITest::MemoryType memType) {
    auto buffer = APITest::UniformBufferRef(new OGLUniformBuffer(memType, initialSize));
...
APITest::VertexBufferRef
APITest::OGLRenderImpl::createVertexBuffer(void *initialData, size_t initialSize, APITest::MemoryType memType) {
    auto ret = std::unique_ptr<VertexBuffer>(new OGLVertexBuffer(memType, initialSize));
...
APITest::OGLRenderImpl::createIndexBuffer(void *initialData, size_t initialSize, APITest::IndexBuffer::Type indexType,
                                          APITest::MemoryType memType) {
    auto ret = std::unique_ptr<IndexBuffer>(new OGLIndexBuffer(memType, initialSize, indexType));
