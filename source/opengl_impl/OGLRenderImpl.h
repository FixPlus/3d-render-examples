...
namespace APITest{
    ...
    public:
        ...
        UniformBufferRef createUniformBuffer(void* initialData, size_t initialSize, MemoryType = MemoryType::HOST_VISIBLE) override;

        DescriptorSetLayoutRef createDescriptorLayout(std::vector<DescriptorLayout> const& desc) override;

        GraphicsPipelineRef createGraphicsPipeline(GraphicsPipelineLayout layout) override;

        ShaderRef createShaderProgram(ShaderDesc const& shaderDesc) override;

        VertexBufferRef createVertexBuffer(void* initialData, size_t initialSize, MemoryType = MemoryType::HOST_VISIBLE) override;
        IndexBufferRef createIndexBuffer(void* initialData, size_t initialSize, IndexBuffer::Type indexType = IndexBuffer::Type::INDEX_TYPE_UINT_32, MemoryType = MemoryType::HOST_VISIBLE) override;
        ...
        FrameStatistics const& statistics() const override{ return statRecorder.stat();};
