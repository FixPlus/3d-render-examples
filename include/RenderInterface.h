...
namespace APITest {
     ...
     *
     *  Global variables that are shared across multiple primitives(vertices or fragments) are called uniforms.
     *
     *  OpenGL 4.2+ core spec allows to have almost identical to Vulkan uniform layout declarations in GLSL.
     *
     *  This wrapper doesn't use global uniforms that are specific for one of the APIs (e.g. PushConstants in Vulkan
     *  and non-block non-opaque uniforms in OpenGL). Neither it uses Vulkan capability to have multiple descriptor
     *  sets (TODO: actually, this would be nice to support.)
     *
     *  */
    ...
    struct DescriptorSetLayout{
        ...
        virtual UniformDescriptorSetRef allocateNewSet(UniformDescriptor* descriptors, int count) = 0;
    ...
    struct RasterizerLayout{
        struct DepthTest{
            bool enable = false;
            bool write = false;
            enum class CompOp { LESS_OR_EQUAL, LESS, GREATER, GREATER_OR_EQUAL} compOp = CompOp::LESS_OR_EQUAL;
        } depthTest;

        struct CullingState{
            enum FrontFace {FRONT_FACE_CW, FRONT_FACE_CCW} face = FRONT_FACE_CCW;
            enum CullMode {CULL_NONE, CULL_FRONT_FACE, CULL_BACK_FACE} cullMode = CULL_BACK_FACE;
        } cullingState;
    };
    ...
    struct CommandRecorder{
        ...
        virtual void draw(uint32_t vertexCount, uint32_t firstIndex) = 0;       // don't use index buffer
        virtual void drawIndexed(uint32_t indexCount, uint32_t firstIndex, uint32_t vertexOffset) = 0; // uses index buffer
    ...
    struct OffscreenRenderPass: virtual public ColorPass{
        /** Defines what to do with attachment contents in the beginning of the pass. Defaults to CLEAR. */
        enum class LoadOp {LOAD, CLEAR, DONT_CARE};
        /** Defines what to do with attachment contents after the pass finishes. Defaults to STORE. */
        ...
        virtual ImageRef createDepthBuffer(uint32_t binding, Image::Format format) = 0;
        virtual ImageRef createColorBuffer(uint32_t binding, Image::Format format) = 0;
    };
    ...
    struct OnscreenRenderPass: virtual public ColorPass{
        ...
        /** Depth buffer is not created by default(in Vulkan). Enable if needed on custom binding index. */
    ...
    public:
        explicit RenderPassRefBase(std::shared_ptr<RenderPass> pass): pass_(std::move(pass)){}

        operator RenderPassRef(){
            return pass_;
        };
    };

    template<typename T>
    struct ChildRenderPassRef: public RenderPassRefBase{

        explicit ChildRenderPassRef(T* allocPass): RenderPassRefBase(std::shared_ptr<RenderPass>(allocPass)){}


        T* operator ->(){
            return dynamic_cast<T*>(pass_.get());
        }
    ...
    public:
        ...
        /** @brief cannot connect more than one window. Previous window will be destroyed.
        ...
        /** @brief create as much images as you wish, but beware of exceeding GPU memory limits. */
        virtual ImageRef createImage(ImageDesc desc) = 0;

        virtual GraphicsPipelineRef createGraphicsPipeline(GraphicsPipelineLayout layout) = 0;

        virtual VertexBufferRef createVertexBuffer(void* initialData, size_t initialSize, MemoryType = MemoryType::HOST_VISIBLE) = 0;
        virtual IndexBufferRef createIndexBuffer(void* initialData, size_t initialSize, IndexBuffer::Type indexType = IndexBuffer::Type::INDEX_TYPE_UINT_32, MemoryType = MemoryType::HOST_VISIBLE) = 0;

        virtual DescriptorSetLayoutRef createDescriptorLayout(std::vector<DescriptorLayout> const& desc) = 0;
        ...
        virtual UniformBufferRef createUniformBuffer(void* initialData, size_t initialSize, MemoryType = MemoryType::HOST_VISIBLE) = 0;
    ...
    struct Window{
        ...
        /** @brief must be called consistently in order to process window events properly and not cause it to hang.
         *  @return true if window hits close condition (e.g. close button clicked). After that window surface becomes
         *  invalid and neither this method nor render() must not be called till window is recreated.
         * */
