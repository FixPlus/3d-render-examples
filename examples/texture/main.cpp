...
APITest::ImageRef loadTexture(APITest::RenderInterfaceRef const& renderInterface, const char* filename) {
    ...
    auto texture = renderInterface->createImage({{static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight), 1, 1},
...
public:
    ...
    static void Init(APITest::RenderInterfaceRef const& renderer, bool use_vulkan) {
        vertexBuffer = renderer->createVertexBuffer(dynamicVertices.data(), dynamicVertices.size() * sizeof (Vertex));
        sampler = renderer->createSampler({});
        descriptorSetLayout = renderer->createDescriptorLayout({{0,
                                                                 APITest::ShaderStage::FRAGMENT,
                                                                 APITest::DescriptorLayout::Type::COMBINED_IMAGE_SAMPLER}});
        APITest::GraphicsPipelineLayout layout;
        layout.vertexLayout.perVertexAttribute = {APITest::VertexLayout::Attribute::RGBA32SF,
    ...
    TexturedRectangle(APITest::RenderInterfaceRef const& renderer, APITest::ImageRef texture): texture_(std::move(texture)) {
    ...
    static void setDynamicVertices(glm::mat4 transform) {
        ...
        std::transform(vertices.begin(), vertices.end(), dynamicVertices.begin(), [transform](Vertex const& in){
            return Vertex{transform * in.pos, glm::vec3(transform * glm::vec4(in.normal, 0.0f)), in.texCoord};
        });

        vertexBuffer->push(dynamicVertices.data(), dynamicVertices.size() * sizeof (Vertex), 0);
    }
...
std::array<TexturedRectangle::Vertex, 6> TexturedRectangle::dynamicVertices = vertices;
...
EXAMPLE_MAIN_HEADER(Texture)
    ...
    std::transform(textures.begin(), textures.end(), std::back_inserter(texturedRectangles), [&renderer](APITest::ImageRef const& texture){
    ...
    renderPass->commands = [&texturedRectangles, &rectToDraw](APITest::CommandRecorder* recorder){
        texturedRectangles.at(rectToDraw).draw(recorder);
    };

    auto gui = renderer->getGUI();

    gui->onKeyDown = [&window, &camera, &example_close, &rectToDraw, rectangleCount, &gui](int key){
        switch(key) {
            ...
            case APITest::KEY_LEFT: rectToDraw = rectToDraw == 0 ? rectangleCount - 1 : rectToDraw - 1; break;
            case APITest::KEY_RIGHT: rectToDraw = (rectToDraw + 1) % rectangleCount; break;
            default: ;
        }
        camera.keyDown(key);
    };
