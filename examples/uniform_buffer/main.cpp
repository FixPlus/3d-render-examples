 ...
 *
 *
 *  This example demonstrates usage of Uniform Buffer Objects (UBO). They are used to pass arbitrary
 *  data to shader that is the same for all primitives. Most commonly they are used to pass some
 *  "global" scene parameters like camera and space transformation matrices.
 *
 *
 * */
...
public:
    ...
    Tetra(APITest::RenderInterfaceRef const& renderer, Camera const& camera, glm::vec3 col,
        ...
        transformMatrixBuffer = renderer->createUniformBuffer(nullptr, sizeof(glm::mat4) + sizeof(glm::vec3));
        ...
        uniformDescriptors[0] = {0, APITest::ShaderStage::VERTEX, transformMatrixBuffer};
        uniformDescriptors[1] = {1, APITest::ShaderStage::VERTEX, camera.getCameraBuffer()};
        descriptorSet = tetraDescriptorLayout->allocateNewSet(uniformDescriptors.data(), uniformDescriptors.size());
    }
    ...
    static void initTetra(APITest::RenderInterfaceRef const& renderer){
        ...
        uniformLayoutDesc.push_back({0, APITest::ShaderStage::VERTEX, APITest::DescriptorLayout::Type::UNIFORM_BUFFER});
        uniformLayoutDesc.push_back({1, APITest::ShaderStage::VERTEX, APITest::DescriptorLayout::Type::UNIFORM_BUFFER});
...
Tetra generateTetra(APITest::RenderInterfaceRef const& renderer, Camera const& camera) {
    return Tetra{renderer, camera,
                 glm::vec3{(std::rand() % 1000) / 2000.0f + 0.5f, 0.0f, (std::rand() % 1000) / 2000.0f + 0.5f},
                 glm::vec3((std::rand() % 1000) / 25.0f - 20.0f,
                           (std::rand() % 1000) / 25.0f - 20.0f,
                           (std::rand() % 1000) / 25.0f - 20.0f),
                 std::pair<float, glm::vec3>{(std::rand() % 1000) / 1000.0f * 0.4f + 0.1f,
