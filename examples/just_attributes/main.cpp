 ...
 * ------------------------------ Just attributes ---------------------------------
 *
 * This example demonstrates the basics of rendering in 3D space. It doesn't use anything more
 * complex than Vertex Attributes - easiest way to pass arbitrary information to the shader.
 * Although such method shows it's inefficiency - all vertex calculation must be performed on CPU.
 *
 * Another thing this example introduce is depth buffer(aka Z-buffer). It is image attachment similar to color
 * attachment but used to store not color information (ouFragColor), but information about fragment's depth.
 * It is common way to "sort" drawn triangles in graphics - that more close one(less Z-coordinate) will override
 * more distant one. In contraire to OpenGL, Vulkan API doesn't have default depth buffer and it should be
 * explicitly created if needed. Downside of using depth buffer is additional memory read/write operations
 * that cost perfomance downgrade. But in fact most of the time it's more cheap than sorting triangle arrays on
 * CPU.
 *
 * */
...
public:
    ...
    Tetra(APITest::RenderInterfaceRef const& renderer, glm::vec3 col,
          glm::vec3 translate_ = glm::vec3{0.0f}, std::pair<float, glm::vec3> rotation = {1.0f, {0.0f, 1.0f, 0.0f}}, glm::vec3 scale_ = glm::vec3{1.0f}):
                        color{col}, translate(translate_), scale(scale_), rotAxis(rotation.second), revPerSecond(rotation.first) {
        vertexBuffer = renderer->createVertexBuffer(vertices.data(),
                                                    vertices.size() * sizeof(PerVertexAttributes),
                                                    APITest::MemoryType::HOST_VISIBLE);
    }

    void update(float time) {
        glm::mat4 rotate = glm::rotate(glm::mat4(1.0f), glm::radians(360.0f * time * revPerSecond),glm::normalize(rotAxis));
        transform = glm::scale(glm::translate(glm::mat4(1.0f), translate) * rotate, scale);
    }
    ...
    void draw(APITest::CommandRecorder* recorder, Camera const& camera) const {
        ...
        std::transform(vertices.begin(), vertices.end(), stagedAttributes.begin(), [this, perspective](PerVertexAttributes const& attr){
            PerVertexAttributes ret;
            ret.normal = perspective * transform *  attr.normal;
            ret.pos =  perspective * transform * attr.pos;
            return ret;
        });
        std::transform(stagedAttributes.begin(), stagedAttributes.end(), stagedAttributes.begin(), [this]( PerVertexAttributes const& attr){
    ...
    static void initTetra(){
        ...
        float lowerPlaneOuterRadius = glm::sqrt(outerRadius * outerRadius - lowerPlaneOffset * lowerPlaneOffset);

        for(int i = 0; i < 3; i++) {
            float angle = glm::radians(static_cast<float>(i) * 120.0f);
            baseVertices.at(i + 1) = glm::vec4(lowerPlaneOuterRadius * glm::sin(angle),
                                               -lowerPlaneOffset,
        ...
        for(int i = 0; i < 3; i++) {
            ...
            glm::vec3 firstBase = vertices.at(i * 3 + 1).pos = baseVertices.at((i + 1) % 3 + 1);
            glm::vec3 secondBase = vertices.at(i * 3 + 2).pos = baseVertices.at(i + 1);
...
Tetra generateTetra(APITest::RenderInterfaceRef const& renderer) {
    return Tetra{renderer, glm::vec3{(std::rand() % 1000) / 2000.0f + 0.5f, 0.0f, (std::rand() % 1000) / 2000.0f + 0.5f},
                 glm::vec3((std::rand() % 1000) / 25.0f - 20.0f,
                           (std::rand() % 1000) / 25.0f - 20.0f,
                           (std::rand() % 1000) / 25.0f - 20.0f),
                 std::pair<float, glm::vec3>{(std::rand() % 1000) / 1000.0f * 0.4f + 0.1f, glm::vec3{(std::rand() % 1000) / 500.0f - 1.0f, (std::rand() % 1000) / 500.0f - 1.0f, (std::rand() % 1000) / 500.0f - 1.0f}},
...
EXAMPLE_MAIN_HEADER(Just Attributes)
    ...
    gui->onKeyDown = [&window, &camera, &paused, &example_close, &gui](int key){
        switch(key) {
            ...
            case APITest::KEY_P: paused = !paused; break;
