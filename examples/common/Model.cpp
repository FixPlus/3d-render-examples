...
namespace Examples{
    ...
    void MMaterial::InitShader(const APITest::RenderInterfaceRef &renderer) {
        if(!fragmentShader)
            fragmentShader = renderer->createShaderProgram({APITest::isVulkan(renderer.get()) ?
                                                        ROOT_SHADERS_DIR"common/gltf.frag.spv" :
                                                        ROOT_SHADERS_DIR"common/gltf.frag",
                                                        APITest::ShaderStage::FRAGMENT});
    }
}
void Examples::MaterialBase::InitMaterialBase(APITest::RenderInterface* renderer){
    defaultMaterialShader = renderer->createShaderProgram(
            { APITest::isVulkan(renderer) ? ROOT_SHADERS_DIR"common/default.frag.spv":
              ROOT_SHADERS_DIR"common/default.frag", APITest::ShaderStage::FRAGMENT});
}

Examples::MeshBase::MeshBase(APITest::RenderInterface* renderer, tinygltf::Model const& model, tinygltf::Mesh const& mesh,
                                        MeshCreateFlags flags,
                                        std::vector<AttributeType> attrMapping): name_(mesh.name) {
    ...
    for(auto& primitive: mesh.primitives) {
        ...
        evalPrimitive.vertexCount = model.accessors[primitive.attributes.find("POSITION")->second].count;
            ...
            for(auto& attr: attrMapping){
                switch (attr) {
                    case AttributeType::POSITION:{
                        ...
                        memcpy(vertexBuf.data() + vertexOffset + localOffset, &pos, sizeof (glm::vec4));
                        localOffset += sizeof (glm::vec4);
                        break;
                    }
                    case AttributeType::NORMAL:{
                        if(bufferNormals)
                            memcpy(vertexBuf.data() + vertexOffset + localOffset, bufferNormals + i * 3, sizeof (glm::vec3));
                        localOffset += sizeof (glm::vec3);
                        break;
                    }
                    case AttributeType::UV:{
                        if(bufferTexCoords)
                            memcpy(vertexBuf.data() + vertexOffset + localOffset, bufferTexCoords + i * 2, sizeof (glm::vec2));
                    ...
                    case AttributeType::COLOR:{
                        ...
                        if(bufferColors){
                            ...
                            color = numColorComponents == 4 ? glm::make_vec4(bufferColors + i * 4) : glm::vec4(glm::make_vec3(bufferColors + i * 3), 0.0f);
                            memcpy(vertexBuf.data() + vertexOffset + localOffset, &color, sizeof (glm::vec4));
                        }
                        localOffset += sizeof (glm::vec4);
                        break;
                    }
        ...
        if(evalPrimitive.indexCount > 0) {
            ...
            switch (accessor.componentType) {
                ...
                default:
                    throw std::runtime_error("[MESH][ERROR] loaded gltf model has invalid index type");
            }
        }
    }
    ...
    vertexBuffer = renderer->createVertexBuffer(vertexBuf.data(), vertexBuf.size(), APITest::MemoryType::GPU_PRIVATE);
    if(totalIndexCount)
        indexBuffer = renderer->createIndexBuffer(indexBuf.data(), totalIndexCount * 4,
...
Examples::MMesh::MMesh(APITest::RenderInterface* renderer, const tinygltf::Model &model,
                       const tinygltf::Node &node, std::vector<MaterialRef *> materials_): MeshBase(renderer, model, model.meshes[node.mesh]), materials(std::move(materials_)) {
...
void Examples::MMesh::resetPipelines(APITest::RenderInterface* renderer) {
    ...
    static APITest::VertexLayout vertexLayout{{APITest::VertexLayout::RGBA32SF,
                                               APITest::VertexLayout::RGB32SF,
                                               APITest::VertexLayout::RG32SF,
                                               APITest::VertexLayout::RGBA32SF,
                                               APITest::VertexLayout::RGBA32SF,
                                               APITest::VertexLayout::RGBA32SF}, 0};
    if(!vertexLayout.vertexShader)
        vertexLayout.vertexShader = renderer->createShaderProgram({
                                                                          APITest::isVulkan(renderer) ? ROOT_SHADERS_DIR"common/gltf.vert.spv": ROOT_SHADERS_DIR"common/gltf.vert", APITest::ShaderStage::VERTEX});

    assert(materials.size() == primitives_.size() && "materials vector must match primitives vector");
    ...
    for(; materialIt != materialEnd; ++materialIt, ++primitiveIt){
        ...
        std::copy(materialDescriptors.begin(), materialDescriptors.end(), uniforms.begin() + 2);
...
void Examples::MMesh::drawInstance(APITest::CommandRecorder *commandRecorder, size_t instanceID) const {
    int i = 0;
...
void Examples::GLTFModel::loadImages(tinygltf::Model &gltfModel) {
    for(auto& image: gltfModel.images){
        ...
        textureDesc.usage = APITest::Image::USAGE_COPY_TO | APITest::Image::USAGE_SAMPLED;
...
void Examples::MMesh::addNewInstance(size_t instanceId, APITest::UniformDescriptor const& transformDescriptor,
    ...
    for(auto& primitive: perPrimitivePipeline){
        ...
        std::copy(materialUniforms.begin(), materialUniforms.end(), uniforms.begin() + 2);
        auto descriptorSet = primitive.second.first->allocateNewSet(uniforms.data(), uniforms.size());
...
Examples::GLTFModel::GLTFModel(const char *filename, Camera& camera): camera_(camera) {

    if(!renderer_)
        throw std::runtime_error("[MODEL][ERROR] trying create model without initializing context GLTFModel::Init()");
    ...
    if(!fileLoaded)
        throw std::runtime_error("[GLTF][ERROR] failed to load model from file " + std::string(filename) + ". " + error);
...
void Examples::GLTFModel::loadMaterials(tinygltf::Model &gltfModel) {
    for (tinygltf::Material &mat : gltfModel.materials) {
        ...
        if (mat.values.find("roughnessFactor") != mat.values.end()) {
            material.materialSettings.roughnessFactor = static_cast<float>(mat.values["roughnessFactor"].Factor());
        }
        if (mat.values.find("metallicFactor") != mat.values.end()) {
            material.materialSettings.metallicFactor = static_cast<float>(mat.values["metallicFactor"].Factor());
        }
        if (mat.values.find("baseColorFactor") != mat.values.end()) {
            material.materialSettings.baseColorFactor = glm::make_vec4(mat.values["baseColorFactor"].ColorFactor().data());
        }
        ...
        if (mat.additionalValues.find("alphaCutoff") != mat.additionalValues.end()) {
            material.materialSettings.alphaCutoff = static_cast<float>(mat.additionalValues["alphaCutoff"].Factor());
        }
...
void
Examples::GLTFModel::loadNode(tinygltf::Model &gltfModel, std::shared_ptr<MNode> parent, const tinygltf::Node &node,
                              uint32_t nodeIndex) {
    ...
    if(node.mesh > -1) {
        ...
        newNode->mesh = std::make_unique<MMesh>(renderer_, gltfModel, node, primitiveMaterials);
    }
...
Examples::GLTFModelInstance Examples::GLTFModel::createNewInstance() {
    ...
    for(auto& node: linearNodes){
        ...
        auto[buffer, emplaced] = node->instanceBuffers.emplace(id, std::pair<MNode::InstanceData, APITest::UniformBufferRef>{data, nullptr});
        buffer->second.second = renderer_->createUniformBuffer(&buffer->second.first, sizeof(MNode::InstanceData));

        APITest::UniformDescriptor bufDesc = {1, APITest::ShaderStage::VERTEX, buffer->second.second};
        APITest::UniformDescriptor cameraDesc = {0, APITest::ShaderStage::VERTEX, camera_.getCameraBuffer()};
        if(node->mesh){
...
void Examples::GLTFModel::drawInstance(APITest::CommandRecorder* recorder, size_t id) {
    for(auto& node: linearNodes){
        if(node->mesh)
