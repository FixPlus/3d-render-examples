...
namespace Examples{
    ...
    public:
        ...
        MeshBase(APITest::RenderInterface* renderer, tinygltf::Model const& model, tinygltf::Mesh const& mesh, MeshCreateFlags flags = 0, std::vector<AttributeType> attrMapping = {});
        ...
        void drawPrimitive(APITest::CommandRecorder* recorder, int index) const{
            ...
            if(primitive.indexCount != 0)
                recorder->drawIndexed(primitive.indexCount, primitive.firstIndex, primitive.firstVertex);
            else
    ...
    public:
        ...
        virtual APITest::RasterizerLayout getRasterizationState() const {
            ...
            cullState.cullMode = APITest::RasterizerLayout::CullingState::CULL_BACK_FACE;
    ...
    public:
        ...
        explicit MMaterial(APITest::RenderInterface* renderer){
            materialBuffer = renderer->createUniformBuffer(&materialSettings, sizeof (materialSettings), APITest::MemoryType::GPU_PRIVATE);
        }
        void compileDescriptors(){
            if(!sampler)
                throw std::runtime_error("[MESH][ERROR] no sampler assigned to material");
            textureDescriptors.clear();
#define BIND_TEXTURE(TEXTURE, BINDING) if(TEXTURE) {\
                    textureDescriptors.push_back({BINDING, APITest::ShaderStage::FRAGMENT, \
                    APITest::CombinedImageSampler{*sampler, *TEXTURE}}); \
                    materialUniformLayout.push_back({BINDING, APITest::ShaderStage::FRAGMENT, APITest::DescriptorLayout::Type::COMBINED_IMAGE_SAMPLER});}
            ...
            textureDescriptors.push_back({9, APITest::ShaderStage::FRAGMENT, materialBuffer});
            materialUniformLayout.push_back({9, APITest::ShaderStage::FRAGMENT, APITest::DescriptorLayout::Type::UNIFORM_BUFFER});
#undef BIND_TEXTURE
        }

        virtual std::vector<APITest::DescriptorLayout> getDescriptorLayout() const override {
    ...
    class MMesh: public MeshBase{
        ...
        std::vector<std::pair<APITest::PipelineRef, std::pair<APITest::DescriptorSetLayoutRef, std::map<size_t, APITest::UniformDescriptorSetRef>>>> perPrimitivePipeline;
    public:
        MMesh(APITest::RenderInterface* renderer, tinygltf::Model const& model, tinygltf::Node const& node, std::vector<MaterialRef*> materials);

        void resetPipelines(APITest::RenderInterface* renderer);

        void drawAll(APITest::CommandRecorder* commandRecorder) const;

        void drawInstance(APITest::CommandRecorder* commandRecorder, size_t instanceID) const;

        void addNewInstance(size_t instanceId, APITest::UniformDescriptor const& transformDescriptor, APITest::UniformDescriptor const& cameraDescriptor);
    ...
    struct MNode{
        ...
        std::map<size_t, std::pair<InstanceData, APITest::UniformBufferRef>> instanceBuffers;
    ...
    class GLTFModel{
        ...
        void loadNode(tinygltf::Model& gltfModel, std::shared_ptr<MNode> parent, const tinygltf::Node& node, uint32_t nodeIndex);
    ...
    public:
        ...
        void draw(APITest::CommandRecorder* recorder) { model_->drawInstance(recorder, id_.value()); };
