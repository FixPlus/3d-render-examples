...
public:
    ...
    explicit Camera(bool flipY): flipY_(flipY){ setPerspective(); setCameraSpace();}


    void allocateCameraBuffer(APITest::RenderInterfaceRef const& renderer) {
        cameraBuffer = renderer->createUniformBuffer(nullptr, sizeof (glm::mat4) * 2);
    }

    APITest::UniformBufferRef getCameraBuffer() const{
        return cameraBuffer;
    }
    ...
    void update(double deltaT){
        ...
        if(cameraBuffer) {
            ...
            cameraBuffer->push(&perspective, sizeof (perspective), sizeof (cameraSpace));
        }
    };

    void keyDown(int key) {
        switch(key) {
            case APITest::KEY_W: keys.up = true; break;
            case APITest::KEY_A: keys.left = true; break;
            case APITest::KEY_S: keys.down = true; break;
            case APITest::KEY_D: keys.right = true; break;
            case APITest::KEY_LEFT_SHIFT: keys.shift = true; break;
            case APITest::KEY_SPACE: keys.space = true; break;
            default: ;
        }
    ...
    void keyUp(int key) {
        switch(key) {
            case APITest::KEY_W: keys.up = false; break;
            case APITest::KEY_A: keys.left = false; break;
            case APITest::KEY_S: keys.down = false; break;
            case APITest::KEY_D: keys.right = false; break;
            case APITest::KEY_LEFT_SHIFT: keys.shift = false; break;
            case APITest::KEY_SPACE: keys.space = false; break;
            default: ;
        }
    }
...
while(!window->handleEvents() && !example_close) { renderer->render();\

#define MAIN_LOOP_TAIL() } renderer->waitIdle();
